...
Функция ВыполнитьЗапрос(ТекстЗапроса, БазаДанных="", ВозвращатьРезультат = Ложь) Экспорт
	...
	Попытка
		...
		Если НЕ ПустаяСтрока(ПараметрыПодключения.ИмяПользователяSQL) Тогда
			Соединение.ConnectionString = Соединение.ConnectionString + "uid=" + ПараметрыПодключения.ИмяПользователяSQL + ";";
			Если НЕ ПустаяСтрока(ПараметрыПодключения.ПарольПользователяSQL) Тогда
				Соединение.ConnectionString = Соединение.ConnectionString + "pwd=" + ПараметрыПодключения.ПарольПользователяSQL + ";";
			КонецЕсли;
		Иначе
			Соединение.ConnectionString = Соединение.ConnectionString + "Trusted_Connection=Yes;";
		КонецЕсли;
...
//
// Параметры:
//		- ВосстановлениеНаДату - ДатаВремя - Дата на котороую подбирается последовательность файлов
//			Если не указана, то берется текущая дата.
//		- МоментВосстановления - ДатаВремя - Возвращаемое значение момента времени,
//			на которое могут быть восстановлены данные с учетом созданных резервных копий
//
// Возвращает:
//		- Массив - Массив файлов в порядке восстановления, если не было ошибок
//		- Неопределено - Если были ошибки
//
Функция ПолучитьСписокФайловДляВосстановленияБД(Знач ВосстановлениеНаДату = "", МоментВосстановления=Неопределено) Экспорт

	Если НЕ ЗначениеЗаполнено(ВосстановлениеНаДату) Тогда
		ВосстановлениеНаДату = ТекущаяДата();
	КонецЕсли;
	
	// Уточним дату восстановления на момент времени (для случая использования Модели восстановления = FULL)
	ТекстЗапроса = "
	|-- Находим Backup loga, содержащий данные на момент времени восстановления
	|SELECT NextBackup.backup_start_date as Result
	|FROM 
	|	(SELECT TOP 1
	...
	|	) AS NextBackup
	|WHERE NextBackup.[type] = 'L'
	|";

	ДатаНужногоЛога =  ПолучитьЗначение(ТекстЗапроса);
	Если ДатаНужногоЛога <> Неопределено Тогда
		ВосстановлениеНаДату = ДатаНужногоЛога +1;
	КонецЕсли;
	...
	Попытка 
		Если Выборка.BOF = Ложь Тогда
			...
			Пока Выборка.EOF = Ложь Цикл
				...
				МоментВосстановления = Выборка.Fields("backup_start_date").Value;
				Выборка.MoveNext();
			КонецЦикла;
		КонецЕсли;
		ВыборкаПрочитана = Истина;
...
//		- МоментВремени - ДатаВремя на которые нужно восстановить данные
//
// Возвращает:
//		- Истина - Если запрос выполнен без ошибок
//		- Ложь - Если были ошибки
//	
Функция ВосстановитьИзРезервнойКопииБД(МассивИменаФайлов, ТаблицаФайловБДИсточника="", МоментВремени=Неопределено) Экспорт
	...
	Если МоментВремени<>Неопределено Тогда
		ТекстЗапроса = ТекстЗапроса + ", STOPAT = '" + Формат(МоментВремени,"ДФ='yyyyMMdd ЧЧ:мм:сс'") +"';";
	КонецЕсли;
	ТекстЗапроса = ТекстЗапроса + "RESTORE DATABASE [" + ПараметрыПодключения.ИмяБазыДанныхSQL + "] WITH RECOVERY;";
...
//******************************************************************
// Возвращает настройку MSSQL-сервера: каталог бэкапов по-умолчанию
//
Функция ПолучитьКаталогБэкаповПоУмолчанию() Экспорт

	ТекстЗапроса = "
	|-- Находим в реестре Каталог Backup-ов по-умолчанию
	...
	|select @BackupDirectory as Result
	|";

	Возврат  ПолучитьЗначение(ТекстЗапроса);
	
КонецФункции

//******************************************************************
// Возвращает модель восстановления БД
//
Функция ПолучитьМодельВосстановленияБД() Экспорт
	
	ТекстЗапроса = "
	|-- Модель восстановления для базы данных
	|SELECT name, recovery_model_desc as Result 
	|FROM sys.databases  
    |  WHERE name = '"+ ПараметрыПодключения.ИмяБазыДанныхSQL +"' 
	|";
...
//******************************************************************
// Получает значение посредством запроса к БД
//
// Параметры:
//		- ТекстЗапроса - Строка - Текст запроса предусматривающего возврат одной строки с полем = "Result"
//		  например: select @BackupDirectory as Result
// Возвращает:
//		- значение поля "Result" первой строки результата запроса
//		- Неопределено - Если результат запроса некорректный 
//
Функция ПолучитьЗначение(ТекстЗапроса) Экспорт
	...
	Попытка 
		Если Выборка.BOF = Ложь Тогда
			Выборка.MoveFirst();
			Результат = Выборка.Fields("Result").Value;
		КонецЕсли;
	Исключение
	КонецПопытки;
